...
npx @modelcontextprotocol/inspector@0.16.1 dotnet run --project src/OrderHub.Mcp

練習3
• 我會透過已啟用的 OrderHub MCP 查詢目前庫存。

• Called orderhub.low_stock({"threshold":5})
  └ [{"Sku": "SKU-1048", "Name": "晨光 行動電源", "StockQuantity": 2}, {"Sku": "SKU-1005", "Name": "極光 筆電支架",
        "StockQuantity": 3}, {"Sku": "SKU-1023", "Name": "雲峰 27吋螢幕", "StockQuantity": 3}, {"Sku": "SKU-1014",
        "Name": "星河 USB-C 集線器", "StockQuantity": 4}, {"Sku": "SKU-1032", "Name": "曜石 機械鍵盤", "StockQuantity":
        4}]
...
- 未接 MCP：agent 無法直接取得目前庫存／需要額外查找。
- 接上 MCP：agent 呼叫 orderhub 的 low_stock 工具，直接取得符合條件的商品。
- 差異：MCP 將既有 service 能力以可探索、可呼叫的工具提供給 agent。

練習4
-取消訂單後回傳 "訂單 1 已取消，庫存已回補"
-在取消同訂單 回傳 "取消失敗：狀態為 Cancelled 的訂單不可取消" 
